...
public class DBHelperWhoHadTheLobster extends SQLiteOpenHelper {
	   ...
	   public DBHelperWhoHadTheLobster(Context context)
	   {
	   ...
	   public boolean insertData (int order_id, String person_name, boolean paid, double paid_amount)
	   {
	      ...
	      db.insert(WHO_HAD_THE_LOBSTER_TABLENAME, null, contentValues);
	      return true;
	   }
	   
	   public Cursor getData(int order_id){
	      SQLiteDatabase db = this.getReadableDatabase();
	      Cursor res =  db.rawQuery( "select * from "+WHO_HAD_THE_LOBSTER_TABLENAME+" where id="+order_id+"", null );
	      return res;
	   }
	   public int numberOfRows(){
	      SQLiteDatabase db = this.getReadableDatabase();
	      int numRows = (int) DatabaseUtils.queryNumEntries(db, WHO_HAD_THE_LOBSTER_TABLENAME);
	      return numRows;
	   }
	   ...
	   public boolean updateData (Integer order_id, String person_name, boolean paid, double paid_amount,String payment_type,double cash_due_back,String signature,String email_id)
	   {
	      ...
	      db.update(WHO_HAD_THE_LOBSTER_TABLENAME, contentValues, "order_id = ? AND person_name= ? ", new String[] { Integer.toString(order_id),person_name } );
	      return true;
	   }
	   ...
	   {
	      SQLiteDatabase db = this.getReadableDatabase();
	      Cursor res =  db.rawQuery( "select * from "+WHO_HAD_THE_LOBSTER_TABLENAME+" where order_id="+order_id+"", null );
	      res.moveToFirst();
	   ...
	   public int getNoPayers(int order_id){
		      SQLiteDatabase db = this.getReadableDatabase();
		      Cursor res =  db.rawQuery( "select order_id from "+WHO_HAD_THE_LOBSTER_TABLENAME+" where order_id="+order_id+"", null );
